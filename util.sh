...
function compile() {
  ...
  local common_args="rtc_include_tests=false treat_warnings_as_errors=false proprietary_codecs=true rtc_use_h264=true ffmpeg_branding=\"Chrome\""
  ...
  pushd $outdir >/dev/null
    ...
    pushd src >/dev/null
      ...
      find $header_source_dir -name '*.h' -o -name README -o -name LICENSE -o -name COPYING | \
        grep './third_party' | \
        grep -E 'boringssl|expat/files|jsoncpp/source/json|libjpeg|libjpeg_turbo|libsrtp|libyuv|ffmpeg|libvpx|opus|protobuf|usrsctp/usrsctpout/usrsctpout' | \
        xargs -I '{}' $CP --parents '{}' $outdir/$package_filename/include
